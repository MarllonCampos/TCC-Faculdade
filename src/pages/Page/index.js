...
function Page({ title, color, border, ...props }) {
  ...
  const { userName, setUserName,  setGreenerys } =
    useContext(UserInfoContext)

  function verificarEmailEPassword() {
    const Toast = ReactSwal.mixin({ // Cria o modelo de pop-up
      toast: true,
      position: 'top-end',
    ...
    if (email == null || email === ''|| password == null || password === ''){ // Se um campo não for preenchido
      return  Toast.fire({ // Mostra o pop-up de erro em cima
        icon: 'error',
  ...
  async function handleClickLogin(){
    ...
    verificarEmailEPassword();

    console.log('Tentando Realizar o Login')

    ReactSwal.fire({ // Aparece a modal de loading para fazer o login
      ...
      if (result.dismiss === ReactSwal.DismissReason.timer){ // Ao demorar mais de 5 segundos mostra modal deerro
        ReactSwal.fire({
          icon:'error',
    ...
    const response = await api.post('/login',{ //Corpo da requisição
      email, 
      password
    }, {
    ...
    if (data.status.toLowerCase() === 'ok') { // Se o resultado da api deu sucesso mostra modal de sucesso
      console.log(data)
      ReactSwal.fire({ // Ativa o modal
        title:'Sucesso',
      ...
      ReactSwal.fire({ // Se a resposta da api não foi satisfatória mostra modal de erro com conteudo do backend
